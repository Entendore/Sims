...
from config import (GRID_SIZE, NUM_SPECIES, DISASTER_INTERVAL,
                    RESOURCE_PULSE_INTERVAL, count_neighbors)
...
def step_events(S, env):
    ...
    # --- Disasters ---
    if S['disaster_freq'] > 0 and S['disaster_cd'] <= 0:
        ...
        S['disaster_cd'] = max(10, int(DISASTER_INTERVAL / S['disaster_freq'])) + np.random.randint(-30, 50)
        S['disaster_flash'] = 8
